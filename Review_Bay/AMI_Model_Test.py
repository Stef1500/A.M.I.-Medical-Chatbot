...
adapter_path = r"C:\Users\L\AMI_Prototype\model"
...
# Base model is already quantized
...
def filter_sources_for_question(user_input, sources):
    """
    Remove alarming / irrelevant results for simple consumer questions.
    """
    q = user_input.lower()
...
def rank_sources(sources, query_type):
    """
    Rank sources by fit for the query type.
    """
...
def retrieve_sources(user_input, query_type):
    """
    Retrieval strategy depends on intent class, not hardcoded symptom fast paths.
    """
    sources = []
...
def format_apa_reference(src):
    ...
    authors_text = ", ".join(authors) if authors else src.get("publisher", "Unknown author")
    ...
    if src.get("source_type") == "PubMed":
        return f"{authors_text}. ({year}). {title}. {container}. {url}"

    if container and container != publisher:
        return f"{authors_text}. ({year}). {title}. {container}. {publisher}. {url}"

    return f"{authors_text}. ({year}). {title}. {publisher}. {url}"


def build_references_block(sources):
    if not sources:
        return ""

    refs = []
    for i, src in enumerate(sources, start=1):
        refs.append(f"[{i}] {format_apa_reference(src)}")
    return "\n".join(refs)
...
while True:
    if not introduced:
        print("AMI: Hello! I’m AMI (Artificial Medical Intelligence). I can provide general health information, but I cannot diagnose medical conditions.")
        introduced = True
    ...
    if query_type == "emergency" or is_emergency(user_input):
        print("AMI: I’m not allowed to diagnose anything, but those symptoms could be serious. Please seek urgent medical care or call emergency services right away.\n")
        continue
    ...
    if query_type in ["common_medical_info", "research_heavy_medical", "unclear"]:
        ...
        references_block = build_references_block(sources)

    """
    if the rules below work properly, replace the contents of formatted_input with this:
    ...
    3. when you use an external source for any part of your response, you will refer to it at the end of the paragraph and you will do reference section at the end of the response (Do not include it if you didn't not use any).
    4. make sure to let the user know that you are NOT allowed to diagnose anything and if they think they really need help they should consult a medical professional
        (you can recommend medicines while reminding the user that you are not allowed to diagnose anything).
